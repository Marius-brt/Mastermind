...
public class MM {
	...
	public static int mancheOrdinateur(int lgCode, char[] tabCouleurs, int numManche,
		...
		Ut.afficherSL("L'ordinateur propose : " + entiersVersMot(cod[0], tabCouleurs));
		...
		while (nbCoups < nbEssaisMax - 1 && rep[nbCoups][0] != lgCode) {
			...
			Ut.afficherSL("L'ordinateur propose : " + entiersVersMot(cod[nbCoups], tabCouleurs));
	...
	public static void main(String[] args) {
		...
		int ordi = 0, humain = 0;
		Ut.afficherSL("--------------");
		for (int i = 1; i < manches + 1; i++) {
			if (i % 2 == 0)
				ordi += mancheOrdinateur(lgCode, tabCouleurs, i, nbEssaisMax);
			else
				humain += mancheHumain(lgCode, tabCouleurs, 0, nbEssaisMax);
			Ut.afficherSL("--------------");
			Ut.afficherSL("Score :");
